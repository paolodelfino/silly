...
import { getContinueWatchingCount } from "@/server/actions";
...
export default function ContinueWatchingLayoutPage({
  children,
}: {
  children?: ReactNode;
}) {
  ...
  const elementsCount = useQuery({
    queryKey: ["continue-watching-elements-count"],
    queryFn: async () => await getContinueWatchingCount(),
  });
    ...
    <div>
        ...
        <div className="absolute left-1/2 -translate-x-1/2">
          {elementsCount.isLoading ? (
            <div className="relative">
              <h1 className="text-large font-semibold">Continue Watching</h1>
      ...
      {(elementsCount.data ?? -1) > 0 && (
        <div className="border-b border-divider px-4 pb-4 pt-1">
          <Search
            path="/continue-watching/search"
            placeholder="Search for titles"
            callback="/continue-watching"
          />
        </div>
      )}
